...
def build_text_graph(cfg, classnames, biomedclip_model, text_encoder):
...
def build_image_graph(cfg, classnames, biomedclip_model, img_encoder, train_loader_x):
...
class MultiModalPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, biomedclip_model, base_text_features, base_img_features):
        ...
        if self.mode == "train" :
            ...
            # base_img_features = self.build_image_graph(cfg, classnames, biomedclip_model, image_encoder, train_loader_x)
        ...
        self.GCN_tt = GraphConvolution(self.hidden_dim, name='knowledgegraph', device=self.dtype, class_num=base_text_features.size()[0])
        self.GCN_it = GraphConvolution(self.hidden_dim, name='knowledgegraph', device=self.dtype, class_num=base_text_features.size()[0])
        self.subgraph_num = 2
        ...
        self.GCN_fuse = GraphConvolution(self.hidden_dim, name='knowledgegraph', device=self.dtype, class_num=base_text_features.size()[0], act=None, dropout = self.dropout, mode="image")
    ...
    def build_prompt_graph(self, text_features, prompts):
        ...
        # edge_text = edge_construct
        #(text_features, enable_mask=True)       # [4, 4, 4]
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, biomedclip_model, train_loader_x):
        ...
        # GraphAdapter
        base_text_features = build_text_graph(cfg, classnames, biomedclip_model, self.text_encoder)
        base_img_features = build_image_graph(cfg, classnames, biomedclip_model, self.image_encoder, train_loader_x)
