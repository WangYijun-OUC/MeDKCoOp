...
def parse_function(*metrics, directory="", args=None, end_signal=None):
    ...
    for subdir in subdirs:
        ...
        files = listdir_nohidden(subdir_path, sort=True)
        ...
        #     print(file)
        for file in files:
            if file.startswith("log.txt"):
                # 构建完整的文件路径
                fpath = osp.join(directory, subdir, file)
        ...
        with open(fpath, "r") as f:
            ...
            for line in lines:
                line = line.strip()     # 去除两端符号

                if line == end_signal:
                    good_to_go = True

                for metric in metrics:      # 评价指标名称和对应正则表达，如"accurancy"
