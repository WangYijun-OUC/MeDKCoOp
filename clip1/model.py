...
class ResidualAttentionBlock_MaPLe(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, design_details=None,
        ...
        self.compound_prompt_nctx = design_details['maple_length']  # 2
        if i == 0:
    ...
    def forward(self, inputs):
        ...
        if type(inputs) == list :
            x = inputs[0]       # [199, 4, 768]
            compound_prompts_deeper = inputs[1]     # 8 * [2, 768]
            counter = inputs[2]
        else :
            x = inputs
            compound_prompts_deeper = None
            counter = 0

        if not self.first_layer :
            if compound_prompts_deeper is not None:
                ...
                if not self.text_layer:
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        # Remove the outputs produced by learnable tokens of previous layer
                        prefix = x[0:x.shape[0] - self.compound_prompt_nctx, :, :]  # [75, 6, 512]
                        # Create/configure learnable tokens of this layer
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        elif current_trainer == 'MaPLe' or current_trainer == 'DCPL':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_MaPLe(width, heads, attn_mask, design_details, text_layer, i)  # 768 12 None True i循环12次
                  for i in range(layers)])
        else:
    ...
    def forward(self, x: torch.Tensor):
        return self.resblocks(x)    # x = combined[]
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "MaPLe" or trainer == "DCPL" :
        ...
        # hyper-parameter if need to add prompt embeddings inside to the input
        # of transformer block or not:
        prompt_till_layer_text = design_details['language_depth']   # {'trainer': 'DCPL', 'vision_depth': 0, 'language_depth': 0, 'vision_ctx': 0, 'language_ctx': 0, 'maple_length': 2}
    ...
    def build_attention_mask(self):
        ...
        [0.,   0.,   0.,   0.,   0.]])
        '''
        mask = torch.empty(self.context_length, self.context_length)    # 77 
